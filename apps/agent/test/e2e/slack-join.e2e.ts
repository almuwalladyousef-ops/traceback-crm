...
async function main() {
	...
	if (!JOINS_FOR_REAL) {
		console.log(
			"NOTE  Traceback joins no channel in this run. A join is permanent.",
		);
	}
	...
	if (!publicUnjoined) {
		record(
			...
			true,
			"skipped: Traceback is already in every public channel",
		);
	...
	if (!privateUnjoined) {
		record(
			...
			true,
			"skipped: no private channel is visible to Traceback",
		);
	} else if (!JOINS_FOR_REAL) {
	...
	record(
		...
		canInvite,
		canInvite
			? "a user token lets Traceback add itself to a private channel"
			: "no user token, so Traceback cannot add itself to a private channel",
	);
