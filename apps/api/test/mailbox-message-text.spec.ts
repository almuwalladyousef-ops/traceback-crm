...
describe("rootMessageId", () => {
	...
	it("gives two mailboxes' copies of one conversation the same root", () => {
		const repA = [
			...
			{ name: "Message-ID", value: "<reply-a@traceback.example>" },
		];
		const repB = [
			{
				name: "References",
				value: "<root@acme.com> <reply-a@traceback.example>",
			},
			{ name: "Message-ID", value: "<reply-b@traceback.example>" },
		];
...
describe("rootMessageIdFrom", () => {
	...
	it("takes the first id when In-Reply-To carries a list", () => {
		expect(
			rootMessageIdFrom({
				...
				messageId: "<reply@traceback.example>",
			}),
		).toBe("a@acme.com");
	});
	...
	it("keeps a multi-id In-Reply-To on the same thread as the single-id copy", () => {
		const listed = rootMessageIdFrom({
			...
			messageId: "<reply-a@traceback.example>",
		});
		const single = rootMessageIdFrom({
			references: null,
			inReplyTo: "<a@acme.com>",
			messageId: "<reply-b@traceback.example>",
		});
