	...
	const join = useMutation(
		trpc.slack.joinChannel.mutationOptions({
			onSuccess: async () => {
				...
				toast.success("Asked someone to invite Traceback.");
			},
			onError: (error) => toast.error(error.message),
		}),
	);
		...
		<div className="flex flex-col gap-9">
			{current ? (
				<Section
					...
					summary="One channel. Traceback joins it when you save."
					title="Lives in"
				>
					<ChannelPicker
			...
			<SaveBar
				description={
					blocked ??
					(channelChanged
						? `Traceback joins #${to}. It stays in #${from} until you remove it.`
