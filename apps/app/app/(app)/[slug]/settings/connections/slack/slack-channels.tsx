...
const INVITE_COMMAND = "/invite @Traceback";
			...
			onSuccess: async (result) => {
				...
				toast.success(
					result.alreadyJoined
						? "Traceback is already in there."
						: result.queued
							? "Traceback is joining."
							: "Ask someone inside to invite Traceback.",
				);
			},
	...
	return (
		<section className="flex flex-col gap-3 px-(--spacing-block-inline)">
			<div className="flex items-end justify-between gap-4">
				<div>
					<h2 className="font-medium text-sm">Channels Traceback can reach</h2>
			...
			{channels.stalled ? (
				<p className="text-warning text-xs">
					Traceback is not reading Slack right now. The list can be out of date.
				</p>
			) : null}
					...
					<p className="px-4 py-4 text-muted-foreground text-sm">
						{channels.pending
							...
							: query
								...
								: "No channels yet. Traceback reads the list from Slack after it connects."}
					</p>
				}
		...
		<AlertDialog open onOpenChange={(open) => !open && onCancel()}>
			<AlertDialogContent>
				<AlertDialogHeader>
					<AlertDialogTitle>
						{canInviteItself
							? `Add Traceback to #${channel.name}?`
							: "Ask someone to add Traceback"}
					</AlertDialogTitle>
					<AlertDialogDescription>
						{canInviteItself
							? `It is a private channel, so Traceback joins as you. Same as typing the invite yourself. Everyone in the channel sees it join. It reads nothing until you turn a permission on.`
							: `We cannot add Traceback to a private channel yet. Someone already in #${channel.name} has to run this.`}
					</AlertDialogDescription>
				</AlertDialogHeader>
				...
				<AlertDialogFooter>
					...
					>
						<AsyncButtonContent pendingLabel="Adding…" status={status}>
							{canInviteItself ? "Add Traceback" : "Copy and mark as asked"}
						</AsyncButtonContent>
					</Button>
				</AlertDialogFooter>
			</AlertDialogContent>
